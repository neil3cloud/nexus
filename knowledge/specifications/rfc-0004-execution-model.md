...
- v1.17 — Adds Reproducible Engineering Session Handoff Binding (Sprint Owner Ratification `NEXUS-RAT-2026-07-19-003`). The delivered `EngineeringSessionHandoff` type (`id`, `missionId`, `sourceEngineeringSessionId`, `sourceRoleId`, `targetEngineeringSessionId`, `targetRoleId`, `recordedAt`, `status`) and the delivered `EngineeringSessionHandoffStatus` closed enumeration (`Recorded` only) are unmodified by this amendment and remain valid under their exact original schema forever. Introduces a new, structurally separate `EngineeringSessionHandoffV1` type (`StructuralOnly` | `ReproducibleContextBound` discriminated union, via a required `handoffContractKind` field) for newly constructed Handoff records only. The `ReproducibleContextBound` variant carries all `StructuralOnly` fields plus exactly one `reproducibleContextPackageReference` (RFC-0003 v1.1 `ContextPackageReference`, reused unmodified, mandatory `pinnedVerificationResultId` included) and an optional `sourceCheckpointId`, owned by Session Recovery/Checkpointing (v1.8, unmodified); a Context Package cannot substitute for a Checkpoint, and Recovery remains exclusively owned by v1.8. This amendment adds no new `EngineeringSessionHandoffStatus` lifecycle value. Before implementing this amendment, the Builder SHALL verify that the selected persistence boundary can store and resolve the delivered `EngineeringSessionHandoff` shape and the additive `EngineeringSessionHandoffV1` variants without ambiguity, destructive rewriting, or identity loss; if it cannot, implementation SHALL stop for a separately authorized persistence-schema or migration decision. Cites RFC-0003 v1.1 only; introduces no dependency on RFC-0008. Depends on `NEXUS-RAT-2026-07-19-001` (RFC-0003 v1.1). No other section of this specification is modified.
...
## Reproducible Context Package Binding (v1.17)

The delivered `EngineeringSessionHandoff` type — `id`, `missionId`, `sourceEngineeringSessionId`, `sourceRoleId`, `targetEngineeringSessionId`, `targetRoleId`, `recordedAt`, `status` — and the delivered `EngineeringSessionHandoffStatus` closed enumeration (`Recorded` only) are unmodified by this amendment and remain valid under their exact original schema, permanently, receiving no conceptual relabeling, no stored default, and no added field.

This amendment defines a separate, structurally distinct type, `EngineeringSessionHandoffV1`, carrying a required `handoffContractKind` discriminator with exactly two values:

- **`StructuralOnly` variant** — `handoffContractKind = "StructuralOnly"`, plus `id`, `missionId`, `sourceEngineeringSessionId`, `sourceRoleId`, `targetEngineeringSessionId`, `targetRoleId`, `recordedAt`, `status` — field-compatible with, but not identical to, the delivered type; used only when a caller explicitly constructs a new Handoff record under this type without binding it to a Context Package.
- **`ReproducibleContextBound` variant** — all `StructuralOnly` fields, plus exactly one `reproducibleContextPackageReference` (RFC-0003 v1.1 `ContextPackageReference`, reused unmodified, mandatory `pinnedVerificationResultId` included) and an optional `sourceCheckpointId`.

`status` values are drawn from the existing, unmodified, closed `EngineeringSessionHandoffStatus` enumeration; this amendment adds no new lifecycle value.

A `sourceCheckpointId`, when present, remains owned by Session Recovery/Checkpointing (v1.8, unmodified); it does not become part of the Context Package, and does not authorize the target session to masquerade as recovery of the source session. A Handoff, in either shape, SHALL NOT embed or duplicate Context Package content, mutate either Engineering Session, recover the target session automatically, select a provider, invoke an Adapter, dispatch execution, advance workflow, create a Workflow Step, alter Assignment Policy, perform planning or deliberation, evaluate Governance, or confer authority.

Fail closed on: missing source or target session; Mission mismatch; invalid role attribution; for `ReproducibleContextBound`, a missing or unresolved required package reference, an unresolved `pinnedVerificationResultId`, package fingerprint mismatch, or incompatible applicability state; missing or incompatible Checkpoint; broken Handoff lineage; duplicate conflicting Handoff identity; unsupported `handoffContractKind`. Recovery remains exclusively owned by Session Recovery/Checkpointing (v1.8, unmodified) and occurs only through an exact Checkpoint; a Context Package cannot substitute for a Checkpoint.

**Builder Stop Condition — Handoff Persistence (binding).** Before implementing this amendment, the Builder SHALL verify that the selected persistence boundary can store and resolve the delivered `EngineeringSessionHandoff` shape and the additive `EngineeringSessionHandoffV1` variants without ambiguity, destructive rewriting, or identity loss. If it cannot, implementation SHALL stop for a separately authorized persistence-schema or migration decision.

This subsection cites RFC-0003 v1.1 only. It introduces zero references to RFC-0008 or its resolution procedure; verification is performed directly under RFC-0003 v1.1's Verification Result mechanism via the mandatory `pinnedVerificationResultId`.
