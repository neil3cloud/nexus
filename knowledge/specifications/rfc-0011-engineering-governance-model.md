...
records were read in. A duplicate fingerprint fails closed, as the governed `Commitment` diagnostic
`duplicate-record-fingerprint`. Two distinct Ratification Authority Records whose
`LifecycleAuthorityRecord` encodings yield the same SHA-256 digest are one such duplicate. This
specification asserts no injectivity and no collision resistance of SHA-256 over record encodings,
and the fail-closed rule is exactly what makes such an assertion unnecessary: a colliding pair is a
governed condition with a governed result, not an implementation defect.
...
- **Fingerprint uniqueness traversal** is record order. The `Commitment` phase SHALL examine record
  fingerprints in record order and SHALL report the first record whose fingerprint equals the
  fingerprint of an earlier record under that order.
...
There are **nine governed phases**, ranked 0 through 8, and a tenth partition,
`ContractViolation` at rank 9, which is not a governed outcome. Ten ranks in total; nine
of them public.
...
| 7 | `Commitment` | public | Records resolved, but the record collection is not committable. |
| 8 | `Envelope` | public | Records resolved and committable, but a declared issuance fact was inadmissible. |
| 9 | `ContractViolation` | not a governed outcome | The implementation violated its own contract. |
...
`Commitment` declares exactly one code, so its within-phase precedence is trivial. Its target
traversal order is record order, stated under Target Selection Order below.

`Commitment` precedes `Envelope`, and the placement is normative. A duplicate record fingerprint is
a defect in the governed source octets and is independent of the declared issuance facts, whereas
every `Envelope` code is attributable to the caller alone. The phase table's shape is: all
governed-source phases first, the single caller-attributable phase last. Were `Commitment` to run
after `Envelope`, a caller's malformed declared fact would mask a governed-source defect, and
whether the governed defect were reported at all would depend on what the caller supplied.

The `Commitment` phase comprises exactly, and in this order: derivation of the authority source
revision from the complete prepared source text; derivation of the record fingerprints in record
order; the record-order fingerprint-uniqueness check; the canonical order-insensitive encoding of
the fingerprint collection; and derivation of the authority root basis and the authority root.
**All five SHALL complete before any `Envelope` examination begins.** Derivation of the envelope
commitment binds declared issuance facts and therefore follows the `Envelope` phase. An
implementation that reports an `Envelope` diagnostic without having completed all five steps does
...
| 7 | `Commitment` | Record order over the record fingerprints; the target is the first record whose fingerprint equals an earlier record's fingerprint. |
| 8 | `Envelope` | The declared-input examination order stated above. |
...
Exactly forty-seven public diagnostic codes are declared. Every one SHALL be reachable
...
`Commitment` — `duplicate-record-fingerprint` (EntryPayload).
...
The three contract-violation codes have no public representation, so one is required for them that
is not a result. The following rules are exact and complete.

1. **Channel.** A contract violation SHALL be surfaced by raising a single named error type
   dedicated to this contract. It SHALL NOT be returned, and no `Issued` or `Rejected` result SHALL
   be produced for it.
2. **Carried classification.** The raised error SHALL carry exactly one of the three
   contract-violation codes and the phase `ContractViolation` at rank 9. Neither the code nor the
   phase SHALL appear in the public diagnostic code partition, the public phase partition, or any
   field of any result.
3. **Reachability.** No governed input SHALL reach this channel. Totality is scoped to governed
   inputs and is preserved exactly: for every governed input the contract returns exactly `Issued`
   or `Rejected`. Unreachability SHALL be established by construction — a closed literal code
   domain, a single validated construction site for payloads, and a governed check preceding every
...
4. **`undeclared-diagnostic`.** The single construction site of a `Rejected` result SHALL verify
   that the supplied code is a member of the declared public vocabulary, and SHALL raise the error
   carrying `undeclared-diagnostic` when it is not. Substituting a governed code for an undeclared
   one SHALL NOT be performed. The rule above that an undeclared code "SHALL be replaced by
   `undeclared-diagnostic` rather than passed through" is the requirement that it never be passed
   through; the replacement it names is this classification, carried on this channel, and not a
   governed result.
...
6. **`internal-invariant-violation`.** An encoder or derivation failure on an already-validated
   fixed schema SHALL raise the error carrying `internal-invariant-violation`. In particular, a
   canonical order-insensitive encoding that refuses the record-fingerprint collection after the
   `Commitment` phase's uniqueness check has admitted it is an internal invariant failure and not a
   governed outcome, because the governed condition it would otherwise report has already been
   decided.

Evidence that a code on this channel is correctly classified MAY be produced by deterministic,
test-only substitution of an internal binding — a digest function or a canonical encoder. Such a
substitution SHALL NOT be reachable from any production call path, SHALL NOT be exposed on any
public entry point, and SHALL NOT be influenced by any governed input or caller-supplied value. A
result obtained under substitution is not a governed input and SHALL NOT be used as agreement
evidence between independent implementations.

Evidence that the phases executed in the order this specification requires MAY likewise be produced
by deterministic, test-only **observation** of an internal binding: a wrapper that records the
inputs presented to a digest function or a canonical encoder, in call order, and returns every
output unchanged. Observation is subject to every restriction stated above for substitution. A
result obtained under observation alone is a governed result, because nothing is substituted; the
recorded trace, however, is an internal observation and SHALL NOT be used as agreement evidence
between independent implementations.

A mechanism that terminates a phase before its later steps execute SHALL NOT be offered as evidence
that those later steps completed. Evidence that a phase completed before a later phase began SHALL
be produced by a mechanism under which the phase in question runs to completion.

An observed trace covers the whole invocation, and an internal binding MAY be exercised by phases
other than the one under examination. Order evidence drawn from such a trace SHALL therefore be
stated as an exact ordered subsequence of the trace — for a terminal phase, its ordered suffix —
and SHALL NOT be stated as a total count of observed calls. A conformance obligation SHALL NOT
require that a binding shared with an earlier phase be unused before the phase under examination.
...
- v1.9 (2026-08-10) — Amended by `NEXUS-RAT-2026-08-10-001` to give the duplicate-record-fingerprint condition a governed public representation and to define the representation of the three non-public contract-violation codes. Adds one governed diagnostic phase, `Commitment`, at rank 7, between `Resolution` and `Envelope`; `Envelope` moves to rank 8 and `ContractViolation` to rank 9. Defines the `Commitment` phase as comprising authority-source-revision derivation, record-fingerprint derivation in record order, the record-order uniqueness check, the canonical order-insensitive encoding of the fingerprint collection, and authority-root derivation, all of which SHALL complete before any `Envelope` examination begins; envelope-commitment derivation continues to follow `Envelope`. Adds one public diagnostic code, `duplicate-record-fingerprint` (EntryPayload), taking the closed public vocabulary from forty-six codes to forty-seven. States that a SHA-256 collision between two distinct record encodings is a governed fail-closed condition, and that no rule in this specification rests on the injectivity or collision resistance of a digest function. Defines the non-public error channel for `undeclared-diagnostic`, `malformed-diagnostic-payload`, and `internal-invariant-violation`, including exact runtime payload validation, and the conditions under which deterministic test-only substitution or observation of an internal binding may evidence that channel and the required phase-execution order, together with the rule that a mechanism terminating a phase before its later steps execute SHALL NOT be offered as evidence that those later steps completed. Adds the fingerprint-uniqueness traversal term and the `Commitment` target selection order. Alters no canonical encoding: `LifecycleAuthorityRecord`, `AuthorityRootBasis`, and `EnvelopeCommitmentBasis` are unchanged field for field, the snapshot schema version remains `nexus-ratification-authority-snapshot/3`, and every fingerprint, root, and commitment is bit-identical for every input that issues under both versions. RFC-0003 is not amended; NCCS-1 rules 6 and 7 are consumed exactly as RFC-0003 v1.1 defines them. Amends `NEXUS-RAT-2026-07-31-001`, `NEXUS-RAT-2026-08-06-001`, and `NEXUS-RAT-2026-08-06-002` by named semantic extent without editing any prior Ledger entry's octets. Implementation remains authorized by `NEXUS-RAT-2026-08-06-002` as so amended.
