...
describe('HostMissionWorkflowCommandRegistration Gemini CLI command', () => {
  it('registers a second Developer Workflow command without changing the MockAdapter command', async () => {
    ...
    new HostMissionWorkflowCommandRegistration(
      ...
      { geminiCliWorkflow: geminiWorkflow },
    );
    ...
    await expect(
      registry.invoke(HOST_RUN_DEVELOPER_MISSION_WORKFLOW_WITH_GEMINI_CLI_COMMAND, {
        objective: 'Run through GeminiCliAdapter.',
        taskTitle: 'Gemini task',
        taskDescription: 'Exercise the production adapter path.',
        adapterExecutionConstraints: {
          'geminiCliAdapter.testDoubleResult': 'Completed',
        },
      }),
  ...
  public async runDeveloperMissionWorkflow(
    ...
    return {
      ...
      knowledge: {
        ...
        approvingAuthority: 'Sprint 30 Gemini CLI command registration test',
        attribution: {
          missionId: `mission-${this.adapterId}`,
          missionPlanRevisionId: 'revision-3',
          supportingEvidenceIds: [`evidence-${this.adapterId}`],
        ...
        provenance: {
          ...
          approvalLineage: 'Sprint 30 Gemini CLI command registration test',
        },
        revisions: [],
      },
    };
