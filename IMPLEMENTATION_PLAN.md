...
Status: 🟡 ACTIVE (Sprint 78 activated — Initial Capability Sequence Step 1)
...
Established by `NEXUS-RAT-2026-07-19-006` as a **binding, planning-only** six-step sequence: its dependency order, certification gates, and per-step compatibility gates bind every future Sprint scoped against it. Step 1 (Sprint 78) is activated by `NEXUS-RAT-2026-07-19-009`; the sequence otherwise authorizes no Sprint, no Builder implementation, and no Sprint Implementation Record by itself. No Steps 2–6 Sprint exists or is activated.

Dependency order: Step 1 → Step 2 → Step 3 (depends on Steps 1–2) → Step 4 (depends on Step 3) → Step 5 (depends on Step 4) → Step 6 (depends on Step 5).

1. **RFC-0002 v1.2 Exact Content Evidence Implementation** — activated by `NEXUS-RAT-2026-07-19-009` as Sprint 78, 🔵 Current Sprint; see `knowledge/implementation/sprints/sprint-0078-rfc-0002-v1.2-exact-content-evidence-implementation.md`. Extends the approved, frozen Sprint 5 Evidence Foundation with `representedContentReference`, content-representation classification, `contentDigestAlgorithm`, `contentDigest`, canonical byte representation, and SnapshotContent/DerivedContent/derivation-source semantics; verified absent from `src/` by direct source search (`EvidenceHash` is an opaque, unvalidated string and does not satisfy `contentDigest`). Binding compatibility gates: no destructive reinterpretation of existing Evidence identities/constructors/snapshots/repositories/persisted records/provenance/version behavior; pre-v1.1 Evidence records must remain distinguishable and fail closed wherever RFC-0013 requires v1.1 Exact Content Evidence. Blocking prerequisite for Steps 3–4.
2. **RFC-0006 v1.2 Assessment Amendment Implementation** — extends the implemented Sprint 9 Review/Finding domain with the generalized `AssessmentSubjectReference`, the exact `AssessmentCriterion`/`AssessmentCriteriaSet` model and fingerprint protocol, Corpus-scoped `AssessmentCoverage`, the Corpus-scoped Finding affected-target model (`SubjectElementTarget`/`AssessmentSubjectTarget`), the Corpus-scoped recorded RFC-0003 Projection basis, and Evidence-expectation enforcement consuming Step 1; verified absent from `src/`. Depends on Step 1. Binding compatibility gates: existing `ReviewPlanRevisionReference` discriminators and consumers preserved unchanged; existing Review/Assessment records not rewritten into `CorpusReviewBasis`; generalization additive or explicitly migrated only under the future Sprint's own authorization; the future Sprint proposal must include compatibility tests covering Sprint 9 and Sprint 74–77 consumers. Blocking prerequisite for Steps 3–4.
