...
Established by `NEXUS-RAT-2026-07-19-006` as a **binding, planning-only** six-step sequence: its dependency order, certification gates, and per-step compatibility gates bind every future Sprint scoped against it, but it authorizes no Sprint, no Builder implementation, and no Sprint Implementation Record by itself. No Sprint 78 (or any Milestone 12 Sprint) exists or is activated. No implementation is authorized.
...
1. **RFC-0002 v1.2 Exact Content Evidence Implementation** — extends the approved, frozen Sprint 5 Evidence Foundation with `representedContentReference`, content-representation classification, `contentDigestAlgorithm`, `contentDigest`, canonical byte representation, and SnapshotContent/DerivedContent/derivation-source semantics; verified absent from `src/` by direct source search (`EvidenceHash` is an opaque, unvalidated string and does not satisfy `contentDigest`). Binding compatibility gates: no destructive reinterpretation of existing Evidence identities/constructors/snapshots/repositories/persisted records/provenance/version behavior; the future Sprint proposal must specify the additive/versioned representation and migration handling; pre-v1.1 Evidence records must remain distinguishable and fail closed wherever RFC-0013 requires v1.1 Exact Content Evidence. Blocking prerequisite for Steps 3–4.
