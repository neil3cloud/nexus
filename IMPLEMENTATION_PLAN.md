...
- `NEXUS-RAT-2026-07-18-004` — declares Milestone 11 Complete on the strength of `NEXUS-REV-2026-07-18-005` (PASS, zero findings) and opens Milestone 12 — Corpus Review and Implementation Readiness.
...
- `NEXUS-RAT-2026-07-18-004` — declares Milestone 11 Complete; opens Milestone 12 with its binding Objective and Architectural Boundary; authorizes RFC-0013's continued architectural drafting and a future final-ratification review; does not ratify RFC-0013, advance its Status/Version, authorize implementation, or activate Sprint 78.
- `NEXUS-RAT-2026-07-19-005` — ratifies RFC-0013 Final, Version 1.0 (no substantive normative architectural change relative to v0.7); does not authorize implementation, does not activate Sprint 78 or any other Milestone 12 Sprint, and does not establish the Milestone 12 Initial Capability Sequence.
- `NEXUS-RAT-2026-07-19-009` — activates Sprint 78 (Initial Capability Sequence Step 1) as Current Sprint on the strength of `NEXUS-RAT-2026-07-19-007`/`-008` (both Active); authorizes the Sprint Implementation Record and Builder Task; redesigns no previously approved scope.
- `NEXUS-RAT-2026-07-21-003` — inserts Corrective Prerequisite 1A between Step 1 and Step 2, defining the future implementation scope for RFC-0002 v1.3 Evidence Confidence and Evidence Verification Status Integration; version-synchronizes Step 2 to RFC-0006 v1.3, including the closed four-variant `EvidenceExpectation` vocabulary and RFC-0002 v1.3 threshold consumption; planning amendment only, no Sprint activation.
- `NEXUS-RAT-2026-07-21-004` — narrow corrective ratification to `NEXUS-RAT-2026-07-21-003`, without silently editing that entry: replaces "authorizes implementation"/"Implements, in full" wording for Corrective Prerequisite 1A (which defines future implementation scope only, per that entry's own no-implementation authorization) and updates Step 4's terminal Assessment reference from RFC-0006 v1.2 to RFC-0006 v1.3, scope otherwise unchanged; planning amendment only, no Sprint activation.
- `NEXUS-RAT-2026-07-21-005` — activates Sprint 79 (Corrective Prerequisite 1A) as Current Sprint, implementing the six-item scope established by `NEXUS-RAT-2026-07-21-003`/`-004` plus the Binding Construction/Reconstitution Contract and Compatibility Test Matrix; authorizes the Sprint Implementation Record and Builder Task; no RFC-0002 amendment, no RFC-0006/Step 2 work, no change to Sprint 78's frozen Evidence Foundation.
- `NEXUS-RAT-2026-07-21-006` — amends the Milestone 12 Initial Capability Sequence: replaces Step 2 with Step 2A (RFC-0006 v1.3 Structural Foundation, dormant runtime, six explicit prohibitions) and inserts new Step 3A (Corpus Assessment Runtime Foundation) between Step 3 and Step 4, gated on four explicit, independently-ratifiable, ownership-attributed stop conditions (Severity/Intent only — no Finding Category); also reconciles all six live Sprint 79 status locations in `IMPLEMENTATION_PLAN.md`/`IMPLEMENTATION_MANIFEST.md` to `NEXUS-REV-2026-07-21-002`'s actual outcome; no Sprint activated; supersedes the withdrawn Sprint 80 Revisions 1–3.
- `NEXUS-RAT-2026-07-21-007` — activates Sprint 80 (Initial Capability Sequence Step 2A) as Current Sprint, per the Sprint 80 Proposal Revision 7 (Sprint Owner APPROVE); authorizes the exact sixteen-file forecasted implementation inventory, the Sprint 80 Sprint Implementation Record, and the Builder Task; does not alter Step 2A's six prohibitions or Step 3A's four independent stop conditions established by `NEXUS-RAT-2026-07-21-006`; no RFC amendment.
- `NEXUS-RAT-2026-07-22-001` — corrects the Milestone 12 Initial Capability Sequence Step 3/Step 3A boundary: narrows Step 3 to exactly the RFC-0013-owned structural value objects requiring neither RFC-0003 Projection identity/freshness, Mission attribution, nor any external resolver, and moves Mission Relationship, Active Evidence Applicability, the nine-component Corpus Review Basis and Basis Fingerprint, `CorpusReview`/`Open`-state construction, cross-Scope/Contract uniqueness, and Assessment Binding into Step 3A; corrects a proposed-text error that would have included the Assessment Criteria Set's identity/version/fingerprint in the Corpus Review Contract Fingerprint (RFC-0013 defines these as separate, non-overlapping components of the Contract Fingerprint and the Basis Fingerprint respectively); requires a separate architectural ratification and Sprint authorization for any future `Other`-variant or external-resolver capability; does not activate any Sprint; no RFC amendment.
- `NEXUS-RAT-2026-07-22-002` — activates Sprint 81 (Initial Capability Sequence Step 3, Narrowed, Final) as Current Sprint, per the Sprint 81 Proposal Revision 6 (final, self-contained; Sprint Owner APPROVE); authorizes the exact sixteen-file forecasted implementation inventory, the Sprint 81 Sprint Implementation Record, and the Builder Task; does not alter Step 3A's four independent stop conditions or authorize any `Other`-variant or external-resolver capability; no RFC amendment.
...
- `NEXUS-RAT-2026-08-06-002` — Stage 2 activation: activates Sprint 82 as Current Sprint on the independent Supporting-Governance Prerequisite Track SGP-1 established by `NEXUS-RAT-2026-08-06-001`; lifts only the `implementation` and `Sprint activation` deferral limbs, and only for the exact scope that entry already defined; creates the Sprint Implementation Record at `knowledge/implementation/sprints/sprint-0082-ratification-authority-snapshot-issuance.md` and opens exactly one Builder task at `builder-task.md`; authorizes exactly four implementation files, thirteen test files, and seven conformance-oracle files, and no other; inserts no step into and alters no part of the six-step Initial Capability Sequence; does not resolve, weaken, or affect Step 3A Stop Conditions 1, 2, or 4; does not reopen completed Milestone 9; does not discharge Dependency DEP1 of `NEXUS-RAT-2026-08-02-001`; preserves the authority-root-pinning deferral in full; broadens no part of DEP2's scope-free-only discharge; corrects no historical narrative text; amends no RFC, no Kernel Canon document, and no prior Ledger entry.

Architectural Boundary (binding, from `NEXUS-RAT-2026-07-18-004`)

Milestone 12 SHALL reuse existing Nexus capabilities and SHALL NOT create a parallel execution or governance architecture. Milestone 12 MAY integrate with Mission, Evidence, Shared Reality, Execution Roles, Adapters, Review, Governance, Knowledge, and Autonomous Planning. Milestone 12 SHALL NOT introduce open-ended AI debate, provider voting, autonomous Canon or RFC amendment, autonomous Mission objective creation, self-authorized implementation, unrestricted artifact mutation, a second Review/Governance/execution model, or Haraya-specific architecture.
...
Dependency order (amended by `NEXUS-RAT-2026-07-21-006`): Step 1 → Corrective Prerequisite 1A → Step 2A → Step 3 (depends on Step 1, Corrective Prerequisite 1A, and Step 2A) → Step 3A (depends on Step 1 + Corrective Prerequisite 1A + Step 2A + Step 3) → Step 4 (depends on Step 3A) → Step 5 (depends on Step 4) → Step 6 (depends on Step 5).
...
Depends on Step 1 only. Binding compatibility gates: no destructive change to Sprint 78's frozen Evidence Foundation (`representedContentReference`, content-representation classification, `contentDigestAlgorithm`, `contentDigest`, canonical byte representation, SnapshotContent/DerivedContent); pre-v1.3 Evidence and Provenance records remain preserved exactly as recorded per item 6 above. Excludes any RFC-0006 concept, any `AssessmentCriterion` or `EvidenceExpectation` implementation, any RFC-0002 Evidence Relationships concept, any Evidence Type registry, and any Canonicalization Profile Registry generalization beyond its existing Exact-Content-Evidence scope. Blocking prerequisite for Step 2A and, transitively, Steps 3–6.

**Activated as Sprint 79 by `NEXUS-RAT-2026-07-21-005`; Approved and fully closed under `NEXUS-REV-2026-07-21-002` (PASS; both prior findings resolved; zero remaining findings).** See `knowledge/implementation/sprints/sprint-0079-corrective-prerequisite-1a-evidence-confidence-and-verification-status.md` for the complete Sprint Implementation Record, including the Binding Construction/Reconstitution Contract and Compatibility Test Matrix.

2A. **RFC-0006 v1.3 Structural Foundation** — established by `NEXUS-RAT-2026-07-21-006`, replacing the prior Step 2 ("RFC-0006 v1.3 Assessment Amendment Implementation"). Scoped exclusively to structural types, deterministic canonicalization, immutable value models, pure Coverage operations, and the pure evaluation result: `AssessmentSubjectReference` (three discriminants; defined in a new standalone file, leaving the existing `ReviewPlanRevisionReference` declaration in `review.types.ts` completely untouched — its conversion to a derived alias is Step 3A's responsibility); `AssessmentCriterion`/`AssessmentCriteriaSet` and its SHA-256 fingerprint protocol; the closed four-variant `AssessmentCriterionApplicability` and `EvidenceExpectation` vocabularies with canonical encoding; `AssessmentCoverage`'s pure universe-computation and disposition-recording operations; `FindingAffectedTarget`'s type (definable, not wired to live `Finding`); `evaluateCoveragePair` as a pure function. Explicitly and exhaustively prohibited: modifying `Review`, `ReviewSnapshot`, or `StartReviewCommand`; any change to `review.events.ts`; any Review-related host/service/repository wiring change; any runtime Corpus Assessment entry point; constructing, persisting, completing, or publishing Findings from a `CorpusReviewBasis` Review; and re-exporting any Step 2A symbol from `review.contract.ts` or any `index.ts` runtime barrel. Depends on Step 1 and Corrective Prerequisite 1A only. Blocking prerequisite for Step 3.

**Activated as Sprint 80 by `NEXUS-RAT-2026-07-21-007`, per the Sprint 80 Proposal Revision 7; Approved and fully closed under `NEXUS-REV-2026-07-22-002` (PASS; all three findings from `NEXUS-REV-2026-07-22-001` resolved; zero remaining findings).** See `knowledge/implementation/sprints/sprint-0080-step-2a-rfc-0006-structural-foundation.md` for the complete Sprint Implementation Record, including the exact sixteen-file forecasted implementation inventory and the full pure Coverage/evaluation contract.

3. **Corpus Review Structural Foundation (Narrowed, Final)** — narrowed by `NEXUS-RAT-2026-07-22-001`, replacing the prior text ("unchanged in scope from the pre-amendment text"). Scoped exclusively to: the six-value `CorpusReviewPurpose` and fourteen-value `CorpusArtifactKind` core enumerations (each with its Canonical Key derivation; `Other` deferred pending its own separate architectural ratification and Sprint authorization); `CorpusArtifactReference` constructed from a caller-supplied `corpusArtifactReferenceId` and a concrete, reconstituted RFC-0002 v1.3 `Evidence` value with confirmed Exact Content Evidence, deriving `artifactId` from that Evidence's `representedContentReference.contentId` (never independently supplied) and every other anchored field directly from that same value; `CorpusReviewScope`/`CorpusReviewContract` each enforcing only within-collection identifier and canonical-exact-identity uniqueness (cross-collection uniqueness deferred to Step 3A); the Contract's Mandatory Assessment Criteria Set Reference, stored on the Contract and derived from a caller-supplied Step 2A `AssessmentCriteriaSet` value but explicitly excluded from the Corpus Review Contract Fingerprint's input set; Corpus Review Opening Attribution, standalone; and the Canonical Fingerprint Protocol applied to the Corpus Review Scope Fingerprint and Corpus Review Contract Fingerprint only — each computed solely over Contract/Scope member canonical exact artifact identities, exactly per RFC-0013, with no Assessment Criteria Set component in either fingerprint. Depends on Step 1, Corrective Prerequisite 1A, and Step 2A. Excludes Mission Relationship, Active Evidence Applicability, the nine-component Corpus Review Basis and Basis Fingerprint, `CorpusReview`/`Open`-state aggregate construction, Assessment Binding, Completion, Withdrawal, Corpus Readiness Result, Reproducible Context Integration, any `Other`-variant implementation, and any external Evidence/Assessment Criteria Set resolver — all relocated to Step 3A or later, or deferred pending separate ratification, by `NEXUS-RAT-2026-07-22-001`.

**Activated as Sprint 81 by `NEXUS-RAT-2026-07-22-002`, per the Sprint 81 Proposal Revision 6; Approved and fully closed under `NEXUS-REV-2026-07-22-005` (PASS; zero remaining findings).** `NEXUS-REV-2026-07-22-003` identified one Major Implementation Defect, `NEXUS-REV-0081-DEF-001` — whitespace-padding acceptance in `CorpusReviewPurpose.fromString`, `CorpusArtifactKind.fromString`, and `CorpusReviewOpeningAttribution`'s `normalizeOriginType`, reintroducing the defect pattern corrected under `BT-079-002` — resolved via `BT-081-002` and independently verified by `NEXUS-REV-2026-07-22-004`, which itself identified one Minor Documentation Drift finding, `NEXUS-REV-0081-DOC-001` (stale Validation Summary counts/missing Implementation Deviations entry), resolved via `DOC-081-002` and independently verified by `NEXUS-REV-2026-07-22-005`. See `knowledge/implementation/sprints/sprint-0081-step-3-corpus-review-structural-foundation-narrowed.md` for the complete Sprint Implementation Record, including the exact sixteen-file implemented inventory and the corrected Contract/Basis Fingerprint separation.
